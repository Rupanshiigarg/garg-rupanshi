...
const projectBorderClasses: Record<string, string> = {
...
const proofColorClasses: Record<string, string> = {
  cyan: "text-cyan",
  pink: "text-pink",
  amber: "text-amber",
  signal: "text-signal",
    ...
    <main className="min-h-screen overflow-hidden bg-ink text-foreground">
      <nav className="fixed inset-x-0 top-0 z-50 border-b border-line/60 bg-ink/85 backdrop-blur-xl" aria-label="Primary navigation">
        <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
          ...
          <Button type="button" variant="ghost" size="icon" className="text-muted-foreground md:hidden" aria-label={menuOpen ? "Close menu" : "Open menu"} onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X size={20} /> : <Menu size={20} />}</Button>
        </div>
      ...
      <section id="top" className="portfolio-grid relative border-b border-line/60 pt-[72px]">
        <div className="absolute inset-0 bg-gradient-to-br from-cyan/[0.035] via-transparent to-pink/[0.06]" />
        <div className="pointer-events-none absolute right-3 top-1/2 hidden origin-right -translate-y-1/2 rotate-90 font-mono text-[9px] uppercase tracking-[0.32em] text-cyan/35 xl:block">Recruiter edition / SDE 2026</div>
        <div className="relative mx-auto grid min-h-[calc(100vh-72px)] max-w-[1440px] items-center gap-14 px-5 py-20 sm:px-8 lg:grid-cols-[1.06fr_0.94fr] lg:gap-16 lg:px-12 lg:py-24">
          <div>
            ...
            <h1 className="max-w-4xl font-display text-[clamp(3.5rem,9vw,8.5rem)] font-medium leading-[0.86] tracking-normal text-foreground">Rupanshi<br /><span className="italic text-cyan">Garg<span className="not-italic text-pink">.</span></span></h1>
            ...
            <div className="mt-9 flex flex-wrap gap-3"><Button asChild size="lg" className="rounded-full bg-cyan px-6 font-mono text-[10px] uppercase tracking-[0.16em] text-ink shadow-none hover:bg-foreground"><a href="#work">View selected work <ArrowDownRight size={15} /></a></Button><Button asChild variant="outline" size="lg" className="rounded-full border-line bg-background/70 px-6 font-mono text-[10px] uppercase tracking-[0.16em] text-foreground shadow-none hover:border-cyan hover:bg-cyan/10 hover:text-cyan"><a href="mailto:gargrupanshi45@gmail.com">Start a conversation <Mail size={14} /></a></Button></div>
          </div>
          <TerminalSignal />
        </div>
